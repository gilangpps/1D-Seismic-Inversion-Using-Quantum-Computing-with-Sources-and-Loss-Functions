...
def build_hamiltonian(mu, rho, dx: float, nx: int):
    ...
    for i in range(nx):
        # Interface μ_{i+½} = ½(μ[i] + μ[i+1]) where μ is BC-padded.
        # Interior point i (padded index i+1) has:
        #   left interface:  μ_{i-½} → μ_half[i]   at μ_bc[min(i,8)]..μ_bc[min(i+1,8)]
        #   right interface: μ_{i+½} → μ_half[i+1] at μ_bc[min(i+1,8)]..μ_bc[min(i+2,8)]
        mu_r = 0.5 * (mu_arr[min(i + 1, len(mu_arr) - 1)]
                    + mu_arr[min(i + 2, len(mu_arr) - 1)])
        mu_l = 0.5 * (mu_arr[min(i,     len(mu_arr) - 1)]
