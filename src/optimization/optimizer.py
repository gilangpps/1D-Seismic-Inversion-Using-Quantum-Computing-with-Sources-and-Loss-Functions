...
class SeismicOptimizer:
    ...
    def __init__(
        ...
        # Reference model m0 for Tikhonov regularization R(mu) = ||mu - m0||^2
        # (Thesis Eq. 2.15; matches g(s) = ||s - s0||^2 in Nguyen & Tura 2025,
        # Eq. 14-15). Defaults to the initial mu (a snapshot taken now, before
        # any Adam updates) if not given explicitly.
        self.mu_prior = (
            np.array(mu_prior, dtype=float) if mu_prior is not None
            else self.mu_arr.copy()
        )
        # Keep the objective's own loss reporting consistent with what is
        # actually being optimized, so J(mu) plotted/logged during
        # optimization already includes the regularization term.
        self.objective.set_regularization(self.reg_weight, self.mu_prior)
    ...
    def _print_iter(self, it, loss, prev_loss, grad, update_norm, overlaps):
        ...
        try:
            ...
            if self.iteration % 25 == 0:
                print(f"  Grad per-index: {np.array2string(grad, precision=2, suppress_small=False, floatmode='unique')}")
                print(f"  mu  per-index:  {np.array2string(self.mu_arr, precision=2, suppress_small=False, floatmode='unique')}")
            print(sep)
    ...
    def run_optimization(self) -> Dict:
        ...
        while self.iteration < self.max_iterations and not self.convergence_reached:
            ...
            # Per-parameter gradient scaling for balanced convergence
            # Interior params (already well-converged) get smaller push;
            # boundary-adjacent params (harder to illuminate) get larger push.
            scale = np.array([0.8, 0.8, 1.0, 1.0, 1.2, 1.5, 2.0])
            grad_scaled = grad * scale

            # Adam step (Bug 2 fix: descent)
            mu_old      = self.mu_arr.copy()
            self.mu_arr = self.adam.step(self.mu_arr, grad_scaled)
