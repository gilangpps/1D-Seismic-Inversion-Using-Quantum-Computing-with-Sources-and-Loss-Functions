...
class SeismicObjective:
    ...
    def __init__(
        ...
        reg_weight : float, default 0.0
            Tikhonov regularization weight lambda in
            J(mu) = data_misfit + lambda * R(mu), R(mu) = ||mu - mu_prior||^2
            (Thesis Eq. 2.15; matches the custom Tikhonov form g(s) = ||s - s0||^2
            in Nguyen & Tura 2025, Eq. 14-15). 0.0 = regularization off (as before).
        mu_prior : array-like or None
            Reference model m0 for the regularization term. If None, no
            regularization term is added even if reg_weight > 0 (falls back
            to misfit-only), since there is no anchor model to penalize
            deviation from.
        """
        self.nx           = nx
        self.dx           = dx
    ...
    def compute(
        ...
        return self.compute_loss(fields, use_deterministic, mu_arr=mu_arr)
    ...
    def compute_loss(
        ...
        J(mu) = (1/T) Sum_t (1/N) ||u_fwd(t; mu) - u_ref(t)||^2
                + reg_weight * ||mu - mu_prior||^2   (Thesis Eq. 2.15, R(m) term)

        The regularization term is only added when mu_arr is provided AND
        self.mu_prior is set (via __init__ or set_regularization()); this
        keeps existing call sites that don't pass mu_arr behaving exactly
        as before (misfit-only).
        """
        if self._reference_fields is None:
            ...
            base_loss = self._reconstruction_loss(fields, use_deterministic)
        else:
            base_loss = self._misfit_loss(fields)

        if mu_arr is not None and self.mu_prior is not None and self.reg_weight > 0.0:
            reg_term = self.reg_weight * float(np.mean((np.asarray(mu_arr, dtype=float) - self.mu_prior) ** 2))
            return base_loss + reg_term
        return base_loss

    def set_regularization(self, reg_weight: float, mu_prior: Optional[np.ndarray]) -> None:
        """Set/replace the Tikhonov regularization weight and reference model m0."""
        self.reg_weight = float(reg_weight)
        self.mu_prior = None if mu_prior is None else np.asarray(mu_prior, dtype=float)

    def _misfit_loss(self, fields: list) -> float:
        """
        Core data-misfit term: compare forward fields against reference.

        Note: the Tikhonov regularization term R(mu) = ||mu - mu_prior||^2
        (Thesis Eq. 2.15) is added in compute_loss(), not here, since it
        needs mu_arr rather than the simulated fields.
        """
    ...
    def compute_loss_and_fields(
        ...
        fields = self.forward_simulate(mu_arr, rho_arr, u0, v0)
        loss   = self.compute_loss(fields, use_deterministic, mu_arr=mu_arr)
        return loss, fields
