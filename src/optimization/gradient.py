...
class FiniteDifferenceGradient:
    ...
    def __init__(
        ...
        delta_scale: float = 1e-3,   # INCREASED: 1e-4 → 1e-3 for better gradient signal
        epsilon: float = 1e6,        # INCREASED: 1.0 → 1e6 Pa for realistic perturbations
    ):
        """
        Parameters
        ----------
        ...
        delta_scale : float
            ...
            Optimized at 1e-4 for balance between FD accuracy and gradient magnitude.
        epsilon : float
    ...
    def compute(
        ...
        """
        Central FD gradient ∂J/∂μ with amplification.

        Amplification strategy: Gradient magnitude may be small due to quantum noise.
        We amplify by (delta / epsilon) to preserve physical significance while
        maintaining optimizer stability.
