...
def run_experiment_1d(
    ...
    u0_bc[1:-1] = u0
    u1_bc = u0_bc.copy()
    u1_bc[1:-1] += dt * v0          # consistent IC: u(dt) ≈ u(0) + dt·v(0)
    ...
    if source_params is not None:
        ...
        src_kind  = source_params.get('kind', 'ricker').lower()
        ...
        elif src_kind == 'gaussian':
            source = gaussian_source(center, width, amplitude, nx + 2,
                                     t0=t0, sigma_t=sigma_t)
        else:
            raise ValueError(f"Unknown source kind: {src_kind}")
